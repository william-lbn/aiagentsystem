# 上游行为复现：OpenAI Agents SDK

- 合同 ID：`UPSTREAM-OPENAI-AGENTS-001`
- 核验状态：`EXTERNAL_NOT_RUN_IN_THIS_RELEASE`
- 固定版本：`openai-agents==0.22.0 @ 4df9ecf`
- 官方来源：https://github.com/openai/openai-agents-python
- 机制焦点：**HITL interruption / durable RunState resume**
...
使用官方 `needs_approval` HITL 路径触发一个有可观察副作用的本地 tool；保存 `RunState`，在新进程/新加载点批准**具体 interruption**并恢复原 run，验证副作用计数恰为 1。

### 故障场景

拒绝该 interruption，或尝试用另一个 call identity 的审批对象替代；验证副作用计数保持 0，run 明确停在拒绝/待处理语义。

### 独立 Verifier

独立 side-effect counter/file + serialized RunState + interruption identity + stdout/trace；不能只看 final_output。
...
此合同验证 SDK 的 pause/approve/resume 行为；它不证明任意外部 API 的 exactly-once。
