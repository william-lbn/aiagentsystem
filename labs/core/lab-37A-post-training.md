# Lab 37A — Post-training 与 Agent 能力塑形｜正常路径

## 实验目标

验证不变量：**post-training evaluation must prevent task leakage between training and held-out evaluation**
...
入口：`examples/chapters/ch37_post_training.py`；核心机制：`src/agentlab/course_scenarios.py::post_training`。

本实验输入由 `post_training` 中固定 fixture 定义，保证每次运行能够比较同一状态转移。
