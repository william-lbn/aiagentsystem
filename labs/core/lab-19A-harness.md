# Lab 19A — Harness 与插件运行时：模型之外的系统能力如何组合｜正常路径

## 实验目标

验证不变量：**plugin activation must follow dependency/lifecycle boundaries instead of hidden global state**
...
入口：`examples/chapters/ch19_harness.py`；核心机制：`src/agentlab/course_scenarios.py::harness`。

本实验输入由 `harness` 中固定 fixture 定义，保证每次运行能够比较同一状态转移。
