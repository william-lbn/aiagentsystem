# Lab 18B — Checkpoint、Journal 与可恢复执行｜故障注入

## 实验目标

验证不变量：**a checkpoint must be atomic and corruption must fail visibly**
...
入口：`examples/chapters/ch18_checkpoint_journal.py`；核心机制：`src/agentlab/course_scenarios.py::checkpoint`。

本实验输入由 `checkpoint` 中固定 fixture 定义，保证每次运行能够比较同一状态转移。
